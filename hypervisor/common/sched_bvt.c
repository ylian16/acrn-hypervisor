...
static void runqueue_add(struct thread_object *obj)
{
	...
	if (list_empty(&bvt_ctl->runqueue)) {
		...
		list_for_each(pos, &bvt_ctl->runqueue) {
			iter_obj = container_of_unsafe(pos, struct thread_object, data);
...
static struct thread_object *sched_bvt_pick_next(struct sched_control *ctl)
{
	...
	if (!list_empty(&bvt_ctl->runqueue)) {
		...
		first_obj = container_of_unsafe(first, struct thread_object, data);
		 ...
		 */
		if (sec != NULL) {
			second_obj = container_of_unsafe(sec, struct thread_object, data);
